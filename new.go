...
type (
	...
	// Driver is what is used to interact with the database. It runs
	...
	Driver struct {
		...
		db      string // the directory where scribble will create the database
...
func New(db string, options *Options) (*Driver, error) {
	...
	return &driver, os.MkdirAll(db, 0755)
}
...
func (d *Driver) Write(collection, resource string, v interface{}) error {
	...
	//
	dir := filepath.Join(d.db, collection)
...
func (d *Driver) Read(collection, resource string, v interface{}) error {
	...
	//
	record := filepath.Join(d.db, collection, resource)
...
func (d *Driver) ReadAll() {
	...
	// ensure there is a database to read
	if d.db == "" {
...
// ReadAll records from a collection; this is returned as a slice of strings because
// there is no way of knowing what type the record is.
func (d *Driver) ReadCollection(collection string) ([][]byte, error) {
...
func readCollection(files []os.FileInfo, dir string) ([][]byte, error) {
	// the files read from the database
	var records [][]byte
